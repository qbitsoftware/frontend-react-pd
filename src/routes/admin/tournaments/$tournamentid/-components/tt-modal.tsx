...
export const TableTennisProtocolModal: React.FC<ProtocolModalProps> = ({ isOpen, onClose, match }) => {
    ...
    const createEmptyPlayers = (count: number, team_number: number, extraData: TableTennisExtraData) => {
        const playerFields: (keyof TableTennisExtraData)[] = [
        ...
        return Array.from({ length: count }).map((_, index) => {
            const playerField = playerFields[index];
    ...
    const usePatchMatch = UsePatchMatch(match.match.tournament_table_id, match.match.id)
    ...
    const handlePlayerChange = async (team: number, index: number, playerId: string) => {
        ...
        let extra_data: TableTennisExtraData
        ...
        if (team === 1) {
            ...
            extra_data = {
                ...
                parent_match_id: "",
                player_a_id: newSelectedPlayers[0].id,
                player_b_id: newSelectedPlayers[1].id,
                player_c_id: newSelectedPlayers[2].id,
                player_d_id: newSelectedPlayers[3].id,
                player_e_id: newSelectedPlayers[4].id,
            }
            ...
            extra_data = {
                table: match.match.extra_data.table,
                parent_match_id: "",
                player_x_id: newSelectedPlayers[0].id,
                player_y_id: newSelectedPlayers[1].id,
                player_z_id: newSelectedPlayers[2].id,
                player_v_id: newSelectedPlayers[3].id,
                player_w_id: newSelectedPlayers[4].id,
            }
            ...
            <DialogContent className="max-w-[350px] md:max-w-[720px] lg:max-w-4xl xl:max-w-6xl h-[80vh] flex flex-col p-0">
                        ...
                        {[1, 2].map((team) => (
                            <div key={team}>
                                ...
                                <div className='mb-[40px] mr-2'>
                                    ...
                                    <Input
                                        type="text"
                                        value={team === 1 ? `${"tiim1 pakpent"}` : `${"tiim 2 kapten"}`}
                ...
                <div className="flex flex-col gap-4 bg-secondary p-6">
                    <div className="flex flex-col md:flex-row gap-4">
                        <Input
                            className='flex-grow'
                            value={match.match.extra_data.table_referee ? match.match.extra_data.table_referee : tablereferee}
                        ...
                        <Input
                            className='flex-grow'
                            value={match.match.extra_data.head_referee ? match.match.extra_data.head_referee : mainreferee}

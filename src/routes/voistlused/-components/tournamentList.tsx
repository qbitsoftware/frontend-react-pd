...
export default function TournamentList({ tournaments }: TournamentProps) {
  ...
  const filteredTournaments = useMemo(() => {
    return tournaments.filter((tournament) => {
      ...
      const categoryMatch = tournament.category
        .toLowerCase()
        .includes(searchTerm.toLowerCase());
      const selectedCategoryMatch =
        !selectedType ||
        tournament.category.toLowerCase() === selectedType.toLowerCase();
      ...
      if (startDate && endDate) {
        ...
        dateMatch =
          isWithinInterval(tournamentStart, {
            start: startDate,
            end: endDate,
          }) ||
      ...
      return (
        ((nameMatch || categoryMatch) && dateMatch) || selectedCategoryMatch
      );
    });
  }, [tournaments, searchTerm, selectedType, startDate, endDate]);

  const groupedTournaments = useMemo(() => {
    return filteredTournaments.reduce(
      (acc, tournament) => {
  ...
  return (
    <div className="container mx-auto p-4 space-y-8 from-blue-50 to-white min-h-screen">
      <h1 className="text-4xl font-bold text-center mb-8 mt-4 md:mt-10">
            ...
            <PopoverTrigger asChild>
              ...
              >
                ...
                {startDate ? (
                  ...
                  <span>{t("tournaments.inputs.start_date")}</span>
                )}
              </Button>
            </PopoverTrigger>
            ...
            <PopoverTrigger asChild>
              ...
              >
                ...
                {endDate ? (
                  ...
                  <span>{t("tournaments.inputs.end_date")}</span>
                )}
              </Button>
            </PopoverTrigger>
            <PopoverContent className="w-auto p-0">
        ...
        {Object.entries(groupedTournaments).map(([type, typeTournaments]) => (
          <div key={type} className="space-y-4">
            <div className="flex justify-between items-center">
              <h2 className="text-2xl md:text-4xl font-semibold text-black/70 capitalize">
                {type}
              </h2>
              <Button
                onClick={() =>
              ...
              >
                {expandedType === type ? (
                  <div>{t("tournaments.show.less")}</div>
                ) : (
                  <div>{t("tournaments.show.more")}</div>
                )}
                <ChevronRight
                  className={`ml-1 h-4 w-4 transform transition-transform ${expandedType === type ? "rotate-90" : ""}`}
                />
              </Button>
            </div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-6">
              {(expandedType === type
                ? typeTournaments
                : typeTournaments.slice(0, 6)
              ).map((tournament) => (
                <Card
                  key={tournament.id}
                  className="overflow-hidden transition-all duration-300 hover:shadow-lg cursor-pointer hover:scale-105 border-blue-200"
                >
                  <Link to={"/voistlused/" + tournament.id} preload="intent">
                    ...
                    <CardContent className="pt-4 p-0">
                      <div className="p-2 md:p-6">
                        ...
                        Formaat:{" "}
                        {capitalize(tournament.category.replace(/_/g, " "))}
                        <div className="">

                ...
                <div className="flex justify-between">
                    <div className="flex">
                        <div className="flex space-x-10">
                            {NavLinks.map((link) => (
                                <Link href={`/voistlused/${params.tournamentid}/${link.href}`} key={link.name} className="text-sm sm:text-base text-white hover:text-gray-200 cursor-pointer">{link.name}</Link>
                            ))}
                        </div>
                    </div>
                </div>

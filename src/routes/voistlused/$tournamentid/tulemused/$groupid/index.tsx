...
function RouteComponent() {
  ...
  useEffect(() => {
    if (tableQuery.data?.data) {
      ...
      if (
        type === GroupType.CHAMPIONS_LEAGUE ||
        type === GroupType.ROUND_ROBIN ||
        type === GroupType.ROUND_ROBIN_FULL_PLACEMENT
      ) {
  ...
  const isRoundRobinFull =
    tournamentType === GroupType.ROUND_ROBIN ||
    tournamentType === GroupType.ROUND_ROBIN_FULL_PLACEMENT
        ...
        >
          ...
          {hasBracketData && (
            <TabsContent value="bracket" className="w-full mt-6">
              ...
              {isRoundRobinFull &&
                bracketQuery.data?.data?.round_robins?.[0] && (
                  <GroupStageBracket
                    brackets={bracketQuery.data.data.round_robins[0]}
                    onMatchSelect={handleSelectMatch}

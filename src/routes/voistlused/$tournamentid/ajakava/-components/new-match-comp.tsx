        ...
        <Card className='w-[300px] sm:w-[300px] xl:w-[350px] p-4'>
            <div className='flex flex-col gap-1'>
                ...
                <div className='flex flex-col gap-2'>
                    <ITTFMatchUserComponent participant={match.p1} match={match} isWinner={match.match.winner_id == match.p1.id} table_data={table_data} />
                    <ITTFMatchUserComponent participant={match.p2} match={match} isWinner={match.match.winner_id == match.p2.id} table_data={table_data} />
                </div>
            </div>
        </Card>
    )
...
const ITTFMatchUserComponent = ({ participant, isWinner, match, table_data }: ITTFMatchUserComponentProps) => {
        ...
        <div className='flex items-center justify-between'>
            <div className='flex items-center gap-3'>
                <Avatar className='h-8 w-8'>
                    <AvatarImage
                        src={table_data.solo ? participant.players[0].extra_data.image_url : participant.extra_data.image_url}
                        className="cursor-pointer object-cover"
                    />
                    <AvatarFallback><img src={placeholderImg} className='rounded-full h-8 w-8' alt="Club" /></AvatarFallback>
                </Avatar>
                <p className={cn(isWinner ? "font-bold" : "", "md:w-32")}>{participant.name}</p>
                {(match.p1.id == participant.id) ? (
                    <span className={cn(isWinner ? "font-bold" : "")}>{match.match.extra_data.team_1_total}</span>
                ) :
                    <span className={cn(isWinner ? "font-bold" : "")}>{match.match.extra_data.team_2_total}</span>
                }
            </div>
            <div className='flex'>
                <div className='flex gap-2 j'>
                    {match.match.extra_data.score && match.match.extra_data.score.map((set, index) => {
                        return (
